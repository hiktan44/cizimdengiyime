    ...
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-lg p-6 flex flex-col items-center gap-6 animate-fade-in backdrop-blur-sm">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-gray-200 flex items-center justify-center gap-2">
          ...
          {isEn ? 'Resolution Upscale' : 'Çözünürlük Yükseltme'}
        </h3>
        <p className="text-sm text-gray-400 mt-2 max-w-md mx-auto">
          {isEn ? 'Sharpen your image and upscale it to 2K or 4K quality using AI.' : 'Yapay zeka kullanarak görselinizi netleştirin ve çözünürlüğünü 2K veya 4K kalitesine yükseltin.'}
        </p>
      </div>

      {(currentWidth && currentHeight) && (
        <div className="bg-black/20 px-4 py-2 rounded-md border border-gray-700/50">
          <span className="text-gray-400 text-sm">{isEn ? 'Current Size: ' : 'Mevcut Boyut: '}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
        ...
        >
          ...
          <span className="text-sm text-gray-400 mt-1">{isEn ? 'Upscale' : 'Yükseltme'}</span>
          <span className="text-xs text-gray-500 mt-2">~2048px</span>
          <span className="text-xs text-blue-400 font-semibold mt-2 bg-blue-500/10 px-2 py-1 rounded">⚡ 1 {isEn ? 'Credit' : 'Kredi'}</span>
        </button>
        ...
        >
          ...
          <span className="text-xs text-amber-400 font-semibold mt-2 bg-amber-500/10 px-2 py-1 rounded">⚡ 2 {isEn ? 'Credits' : 'Kredi'}</span>
        </button>
      </div>
    </div>
  );

  ...
  return (
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-2xl p-5 flex flex-col gap-6 animate-fade-in backdrop-blur-md">
      <div className="flex items-center justify-between px-1">
        <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400">{isEn ? 'Filter Selection' : 'Filtre Seçimi'}</h3>
        {hasAnySelection && (
          <button onClick={handleReset} className="text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors uppercase">{isEn ? 'Reset' : 'Sıfırla'}</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
        {presets.map(preset => (
          <button
            ...
            disabled={isLoading}
            className={`w-full text-center py-3 px-4 rounded-xl transition-all duration-200 border text-sm font-bold ${selectedPresetPrompt === preset.prompt
      ...
      <div className="space-y-4 bg-black/20 p-5 rounded-2xl border border-white/5">
        <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-2">{isEn ? 'Customize Filter Character' : 'Filtre Karakterini Özelleştir'}</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
          {renderSlider(isEn ? 'Intensity' : 'Yoğunluk', 'intensity', 0, 100)}
          {renderSlider(isEn ? 'Saturation' : 'Doygunluk', 'saturation', -100, 100)}
          {renderSlider(isEn ? 'Contrast' : 'Kontrast', 'contrast', -100, 100)}
          {renderSlider(isEn ? 'Hue' : 'Renk Tonu', 'hue', -50, 50, '°')}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <input
          ...
          placeholder={isEn ? 'Or type a custom style (e.g., Pastel dream)' : 'Veya özel bir stil yazın (örn: Pastel rüyası)'}
        ...
        <button
          onClick={handleApply}
          className={`w-full py-4 px-6 rounded-xl font-bold transition-all duration-300 shadow-xl flex items-center justify-center gap-2 ${hasAnySelection && !isLoading
        ...
        >
          {isLoading ? (
            ...
            <>
              {isEn ? 'Apply Filter' : 'Filtreyi Uygula'}

  ...
  return (
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex flex-col items-center gap-4 animate-fade-in backdrop-blur-sm">
      <div className="flex justify-between w-full items-center">
        <h3 className="text-lg font-semibold text-gray-300">{isEn ? 'Crop & Rotate' : 'Kırp & Döndür'}</h3>
      </div>

      {/* Rotation Controls */}
      <div className="w-full bg-black/20 p-3 rounded-lg flex flex-col gap-3">
        <div className="flex items-center justify-between gap-2">
          <span className="text-sm font-medium text-gray-400">{isEn ? 'Rotate:' : 'Döndür:'}</span>
      ...
      <div className="w-full flex items-center gap-4 bg-black/20 p-3 rounded-lg">
        <span className="text-sm font-medium text-gray-400">{isEn ? 'Flip:' : 'Çevir:'}</span>
        <div className="flex gap-2">
          <button
          ...
          >
            <FlipHorizontalIcon className="w-5 h-5" /> {isEn ? 'Horizontal' : 'Yatay'}
          </button>
          <button
          ...
          >
            <FlipVerticalIcon className="w-5 h-5" /> {isEn ? 'Vertical' : 'Dikey'}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center flex-wrap justify-center gap-2 w-full">
        {aspects.map(({ name, value }) => (
          <button
            ...
            disabled={isLoading}
            className={`px-3 py-2 rounded-md text-sm font-semibold transition-all duration-200 active:scale-95 disabled:opacity-50 ${activeAspect === name

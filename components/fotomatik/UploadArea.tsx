  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        alert(isEn ? "Image file could not be processed." : "Görüntü dosyası işlenemedi.");
      }
    }
  };
  ...
  return (
    <div className="w-full">
      <label className="block text-sm font-medium text-slate-300 mb-2">
        {isEn ? 'Source Image' : 'Kaynak Görüntü'}
      </label>
      ...
      {!selectedImage ? (
        ...
        >
          ...
          <p className="text-slate-300 font-medium">{isEn ? 'Click to upload a photo' : 'Fotoğraf yüklemek için tıklayın'}</p>
          <p className="text-slate-500 text-sm mt-1">{isEn ? 'JPG, PNG supported' : 'JPG, PNG desteklenir'}</p>
          <input
            type="file"
            ref={fileInputRef}
        ...
        <div className="relative rounded-xl overflow-hidden border border-slate-700 h-64 bg-black/40 flex items-center justify-center group">
          ...
          <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              title={isEn ? 'Edit Image' : 'Görüntüyü Düzenle'}
            >
            ...
            <button
              ...
              title={isEn ? 'Remove Image' : 'Görüntüyü Kaldır'}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
